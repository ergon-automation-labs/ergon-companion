pipeline {
  ...
  environment {
    BOT_NAME = 'companion_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-companion"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/companion_bot/bin/companion_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyCompanion.Release.create()'"
          $RELEASE_BIN eval 'BotArmyCompanion.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyCompanion.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyCompanion.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_companion/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_companion/releases/start_erl.data)
        fi
