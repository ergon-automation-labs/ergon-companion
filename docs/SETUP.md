# Getting Started with bot_army_companion

This guide walks you through setting up Companion Bot for local development.
...
DATABASE_NAME=bot_army_companion_dev
```
...
Companion Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_companion/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_companion/skills/example.ex lib/bot_army_companion/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyCompanion.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_companion.command.classify"]
...
```elixir
# In lib/bot_army_companion/application.ex or similar

defmodule BotArmyCompanion.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyCompanion.Skills.Classify,
      BotArmyCompanion.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_companion/skills/my_skill.ex
...
releases: [
  companion_bot: [
    applications: [bot_army_companion: :permanent]
  ]
]
```

This creates a release named `companion_bot` that is deployed to `/opt/ergon/releases/companion_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/companion_bot/`
